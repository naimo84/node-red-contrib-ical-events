        ...
        if (todayOnly || !alreadyStarted) {
            ...
            if (config.replacedates) {
                if (_class === 'ical_today')
                    return { text: ((alreadyStarted && !todayOnly) ? '&#8594; ' : '') + _('today', config) + _time, _class: _class };
                if (_class === 'ical_tomorrow')
                    return { text: (alreadyStarted ? '&#8594; ' : '') + _('tomorrow', config) + _time, _class: _class };
                if (_class === 'ical_dayafter')
                    return { text: (alreadyStarted ? '&#8594; ' : '') + _('dayafter', config) + _time, _class: _class };
                if (_class === 'ical_3days')
                    return { text: (alreadyStarted ? '&#8594; ' : '') + _('3days', config) + _time, _class: _class };
                if (_class === 'ical_4days')
                    return { text: (alreadyStarted ? '&#8594; ' : '') + _('4days', config) + _time, _class: _class };
                if (_class === 'ical_5days')
                    return { text: (alreadyStarted ? '&#8594; ' : '') + _('5days', config) + _time, _class: _class };
                if (_class === 'ical_6days')
                    return { text: (alreadyStarted ? '&#8594; ' : '') + _('6days', config) + _time, _class: _class };
                if (_class === 'ical_oneweek')
                    return { text: (alreadyStarted ? '&#8594; ' : '') + _('oneweek', config) + _time, _class: _class };
            }
        }
        else {
            _class = 'ical_today';
