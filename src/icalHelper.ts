...
export default class icalHelper {
    ...
    static  insertSorted(arr, element) {
        if (!arr.length) {
            arr.push(element);
        } else {
    ...
    static formatDate(_date, _end:Date, withTime, fullday, config) {
        ...
        if (todayOnly || !alreadyStarted) {
            ...
            if (config.replaceDates) {
                ...
                if (_class === 'ical_3days') return { text: (alreadyStarted ? '&#8594; ' : '') + this._('3days') + _time, _class: _class };
                if (_class === 'ical_4days') return { text: (alreadyStarted ? '&#8594; ' : '') + this._('4days') + _time, _class: _class };
                if (_class === 'ical_5days') return { text: (alreadyStarted ? '&#8594; ' : '') + this._('5days') + _time, _class: _class };
                if (_class === 'ical_6days') return { text: (alreadyStarted ? '&#8594; ' : '') + this._('6days') + _time, _class: _class };
